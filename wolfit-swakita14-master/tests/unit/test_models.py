...
def test_comment_as_string():
        ...
        assert body in str(new_comment)
...
def test_a_user_can_only_vote_down_once(client, test_user, single_post):
    single_post.down_vote(test_user)
...
def test_comments_can_be_voted_down_on(client, test_user, single_post_with_comment):
    comment = single_post_with_comment.comments[0]
    new_user = User(username="robot", email="robot@gmail.com")
    db.session.add(new_user)
    db.session.commit()
...
def test_user_cannot_change_vote_count_down_for_own_comment(
    client, test_user, single_post_with_comment
):
    c = single_post_with_comment.comments[0]
    assert c.vote_count == 1
