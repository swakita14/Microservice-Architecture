...
def test_create_post_when_not_authenticated(client, test_user, default_category):
    login(client, test_user.username, PASSWORD2)
    response = client.get(url_for("create_post"))
    assert response.status_code == 302
    assert "/register" in response.headers["Location"]

def test_server_shutdown(client):
    return client.get(url_for("shutdown"))
